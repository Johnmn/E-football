...
$status=2;
$cancelby='a';
$sql = "UPDATE matches SET status=:status,CancelledBy=:cancelby WHERE  BookingId=:bid";
...
$msg="Match Cancelled successfully";
}
...
$status=1;
$cancelby='a';
$sql = "UPDATE matches SET status=:status WHERE BookingId=:bcid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
$query-> bindParam(':bcid',$bcid, PDO::PARAM_STR);
$query -> execute();
$msg="Match Confirm successfully";
}
...
<!--heder end here-->
<ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="index.html">Home</a><i class="fa fa-angle-right"></i>Manage matches</li>
            </ol>
...
{				?>		
						  <tr>
							...
							<td><?php echo htmlentities($result->Clubs);?></td>
							<td><?php echo htmlentities($result->Venue);?></td>
							
<td><a href="delete.php?bkid=<?php echo htmlentities($result->bookid);?>" onclick="return confirm('Do you really want to cancel match')" >Cancel</a>  <a href="delete.php?bckid=<?php echo htmlentities($result->bookid);?>" onclick="return confirm('Do you really want to cancel match')" ></a></td>
<?php $cnt=$cnt+1; } ?>
